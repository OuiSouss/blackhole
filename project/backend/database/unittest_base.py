...
class BaseTest(unittest.TestCase):
	...
	def test_add_route(self):
		post = {'ip': 'test_ip', 'next_hop' : 'test_nexthop', 'communities' : 'test_commu'}
		...
		self.assertEqual(post2['next_hop'], post['next_hop'], 'insertion failed')
		self.assertEqual(post2['communities'], post['communities'], 'insertion failed')
		self.database.delete_route({'ip':post['ip']})

	def test_get_all_routes(self):
		post = {'ip': 'test_ip', 'next_hop': 'test_nexthop', 'communities': 'test_commu'}
		post2 = {'ip': 'test_ip2', 'next_hop': 'test_nexthop2', 'communities': 'test_commu2'}
		self.database.add_route(post)
		...
		for r in post3:
			if r['ip']==post['ip']:
				...
				self.assertEqual(r['next_hop'], post['next_hop'], 'insertion failed')
				self.assertEqual(r['communities'], post['communities'], 'insertion failed')
			else :
				self.assertEqual(r['ip'], post2['ip'], 'insertion failed')
				self.assertEqual(r['next_hop'], post2['next_hop'], 'insertion failed')
				self.assertEqual(r['communities'], post2['communities'], 'insertion failed')
		self.database.delete_route({'ip':post['ip']})
